...
export const ClienteScreen = ({ history }) => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3001/clientes/${clienteId}`).then((resp) => {
    ...
    if (bandera) {
        axios.get(`http://localhost:3001/clientes/estados/${cliente.cuit_cuil}`).then((resp) => {
    ...
    if (
        ...
        axios.get("http://localhost:3001/general/clienteScreen").then((resp) => {
